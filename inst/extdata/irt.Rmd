...
This content has been loaded from the template report `.Rmd` file, provided with the `WEIN` package. Please edit it at your best convenience!
...
if(!is.null(values$res_obj)) {
  ...
  print(values$choose_expfac)

  fac1 <- values$choose_expfac
...
if(!is.null(values$res_obj)) {
  ...
  p1 <- ggplot(res_df, aes(x=.data[["pvalue"]])) +
  ...
  p2 <- ggplot(res_df, aes(x=.data[["pvalue"]])) +
  ...
  p4 <- ggplot(res_df, aes(x=.data[["log2FoldChange"]])) +
  ...
  if (!is.null(input$speciesSelect) && input$speciesSelect %in% annoSpecies_df$species) {
    ensembl_species <- annoSpecies_df$ensembl_db[match(input$speciesSelect, annoSpecies_df$species)]
...
if(!is.null(values$res_obj)) {
  ...
  if(!is.null(input$ma_brush)){
    if(!is.null(values$annotation_obj))
        ...
        geom_text(aes(label=.data[["genename"]]),size=3,hjust=0.25, vjust=-0.75))
      else
        print(plot_ma(values$res_obj,annotation_obj = values$annotation_obj,FDR = input$FDR) +
        coord_cartesian(xlim = c(input$ma_brush$xmin,input$ma_brush$xmax),
                        ylim = c(input$ma_brush$ymin,input$ma_brush$ymax)))
...
`WEIN` (Web-based Engine for Interactive Next-generation sequencing analysis) is a Bioconductor package containing a Shiny application for
 interactive and reproducible Differential Expression analysis.

`WEIN` is developed by Bernd Jagla at the Institut Pasteur, based on the original `ideal` package developed in the Bioinformatics Division (led by Harald Binder)
...
`WEIN` is currently developed by Bernd Jagla at the Institut Pasteur. You can contact him by clicking on the button below.

<a href="mailto:bernd.jagla@pasteur.fr?subject=[WEIN_feedback]" class="btn btn-primary">Bernd Jagla</a>

## Code availability

`WEIN` is available on <a href="https://github.com/baj12/WEIN" target="_blank">GitHub</a>.
  ...
  tags$div(
    ...
    tags$div(
      ...
      list(
        ...
        tags$a(href="https://github.com/baj12/WEIN", "WEIN"), br(),
        "WEIN is a project developed by Bernd Jagla at the Institut Pasteur, based on the original ideal package by Federico Marini in the Bioinformatics division of the ",
