...
package: "`r BiocStyle::pkg_ver('WEIN')`"
abstract: >
  In the scope of differential expression analysis, we provide the R/Bioconductor 
  package `r Biocpkg("WEIN")`, which serves as a web application to allow
  simultaneously for interactive and reproducible analysis. `r Biocpkg("WEIN")`
...
output: 
  BiocStyle::html_document:
    toc_float: true
vignette: >
  %\VignetteIndexEntry{WEIN User's Guide}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
  %\VignettePackage{WEIN}
...
<!-- **Package**: `r Rpackage("WEIN")` -->

<!-- **Authors**: `r packageDescription("WEIN")[["Author"]]` -->

<!-- **Version**: `r packageDescription("WEIN")$Version` -->

**Compiled date**: `r Sys.Date()`

**Last edited**: 2019-02-06

**License**: `r packageDescription("WEIN")[["License"]]`
...
<p align="center">
<img src="WEIN_logo_v2.png"/>
</p>

`r Biocpkg("WEIN")` is an R package distributed as part of the [Bioconductor](http://bioconductor.org) project. 
...
The GitHub repository for `r Biocpkg("WEIN")` is https://github.com/baj12/WEIN. 
This is the place to file an issue, report a bug, or provide a pull request.

Once `r Rpackage("WEIN")` is installed, it can be loaded by the following command.
...
`r Biocpkg("WEIN")` is a Bioconductor package containing a Shiny application for analyzing RNA-Seq data in the context of differential expression. 
...
`r Biocpkg("WEIN")` is an R package which fully leverages the infrastructure of the Bioconductor project in order to deliver an interactive yet reproducible analysis for the detection of differentially expressed genes in RNA-Seq datasets. 
...
If you use `r Biocpkg("WEIN")` for your analysis, please cite it as here below:
...
There are different ways to use `r Biocpkg("WEIN")` for interactive differential expression analysis.

## Launching `r Biocpkg("WEIN")` locally
...
and then launch the app with the `WEIN` function.
...
- `WEIN(countmatrix = countmatrix, expdesign = expdesign)`, where instead of passing the defined `DESeqDataSet` object, its components are given, namely the count matrix (e.g. generated after a run of featureCounts or HTSeq-count) and a data frame with the experimental covariates.
  The design formula can be constructed interactively at runtime
- `WEIN()`, where the count matrix and experimental design can simply be uploaded at runtime, where all the derived objects can be extracted and computed live.
...
## Accessing the public instance of `r Biocpkg("WEIN")` 

To use `r Biocpkg("WEIN")` without installing any additional software, you can access the public instance of the Shiny Server made available at the Institute of Medical Biostatistics, Epidemiology and Informatics (IMBEI) in Mainz.

This resource is accessible at this address: 

http://shiny.imbei.uni-mainz.de:3838/WEIN

## Deploying to a Shiny Server

A deployment-oriented version of the package is available at https://github.com/baj12/WEIN. 
This repository contains also detailed instruction to setup the running instance of a Shiny Server, where `r Biocpkg("WEIN")` can be run without further installation for the end-users.

Please note that you still need `r Biocpkg("WEIN")` to be installed there once during the setup phase - for this operation, you might require root administrator permissions. 
...
For obtaining the best analysis experience in `r Biocpkg("WEIN")`, it is recommended to provide all of them.
...
- `Exit WEIN & save` will close the application and store the content of the `input` and `values` reactive objects in a list of two elements in the `ideal_env` environment, respectively called `WEIN_inputs_YYYYMMDD_HHMMSS` and `WEIN_values_YYYYMMDD_HHMMSS`
- `Save State as .RData` will similarly store `LiveInputs` and `r_data` in a binary file named `WEINState_YYYYMMDD_HHMMSS.Rdata`, without closing the application
           
# The main app panels

The `r Biocpkg("WEIN")` app is a one-paged dashboard, structured in different panels, where each of them is focused on a different aspect of the data exploration. 
...
- to tighten the concert operations between similar tools with different scope (as `r Biocpkg("pcaExplorer")` and `r Biocpkg("WEIN")` are), the information flow can move from the data exploration to decisions taken at the moment of testing
...
Additionally, `r Biocpkg("WEIN")` has an option to include a filter step at the gene level by removing genes with low absolute or averages low values. 
...
To avoid repeating this manually, the user can also quickly upload a list of genes as text file (one gene identifier per line), such as members of gene families (e.g. all cytokines, all immunoglobilines, ...) or defined by common function (e.g. all housekeeping genes, or others based on any annotation).
...
This panel of WEIN has a slim interface to the following methods for performing functional analysis:
...
`r Biocpkg("WEIN")` allows the user to work simultaneously with more gene lists, two of which can be uploaded in a custom way (e.g. list of gene families, or extracted from other existing publications). 
...
The About tab contains the output of `sessionInfo`, plus general information on `r Biocpkg("WEIN")`, including the link to the Github development version. 
...
# Running `r Biocpkg("WEIN")` on an exemplary data set

We can run `r Biocpkg("WEIN")` for demonstration purpose on published datasets that are available as SummarizedExperiment in an experiment Bioconductor packages.
...
To run `r Biocpkg("WEIN")` on this dataset, the following commands are required. First, prepare the objects to be passed as parameters of `r Biocpkg("WEIN")`.
...
WEIN(dds_obj = dds_airway)
# or also providing the results object
WEIN(dds_obj = dds_airway,res_obj = res_airway)
```
...
Let's suppose you performed part of your analysis with `r Biocpkg("edgeR")` or `r Biocpkg("limma")`/`voom` and you want to continue inspecting your data interactively using `r Biocpkg("WEIN")`. 
...
```{r WEIN-edgerlimma, eval=FALSE}
WEIN(redo_dds_airway,edger_resu)
# or ...
WEIN(redo_dds_airway,limma_resu)
```
...
The functions exported by the `r Biocpkg("WEIN")` package can be also used in a standalone scenario, provided the required objects are in the working environment. 
...
myde_enhanced <- myde
myde_enhanced$id <- WEIN:::createLinkENS(myde_enhanced$id,species = "Homo_sapiens")
myde_enhanced$symbol <- WEIN:::createLinkGeneSymbol(myde_enhanced$symbol)
...
mygo_enhanced$category <- WEIN:::createLinkGO(mygo_enhanced$category)
...
The MA plot provided by `r Biocpkg("WEIN")` displays the gene-wise log2-fold changes (logFCs) versus the average expression value. 
...
anyfile <- system.file("extdata/design_tabs.txt",package = "WEIN") # we know it is going to be TAB
...
Additional functionality for the `r Biocpkg("WEIN")` will be added in the future, as it is tightly related to a topic under current development research. 
